...
def timer(outputWindow):
    while True:
        if Vars.timer>0:
            slp(1)
            display(outputWindow)
            Vars.timer-=1
        else:
            spawnFruit()
            display(outputWindow)
...
def gameover(outputWindow):
    ...
    print(color.RED+color.BOLD+"GAMEOVER! "+color.END+color.END)
    print(color.GREEN+color.BOLD+"Score: "+str(Vars.score)+color.END+color.END)

    try:
        h=open('highscore.txt', 'r')
        highscore=h.readline()
        h.close()
        if int(highscore)>Vars.score:
            print(color.RED+color.BOLD+"HighScore: "+highscore+color.END+color.END)
        else:
            print(color.GREEN+color.BOLD+"New HighScore: "+str(Vars.score)+color.END+color.END)
            h=open('highscore.txt', 'w')
            h.write(str(Vars.score))
            h.close()
    except:
        print(color.GREEN+color.BOLD+"New HighScore: "+str(Vars.score)+color.END+color.END)
        h=open('highscore.txt', 'w')
...
def move(outputWindow):
    while True and Vars.gameover==False:
        slp(Vars.delay)
...
def display(outputWindow):
    ...
    outputWindow.tag_config("player", background="black", foreground="green")
    outputWindow.tag_add("fruit", str(Vars.posRF+3)+"."+str(Vars.posCF+1), str(Vars.posRF+3)+"."+str(Vars.posCF+2))
    outputWindow.tag_config("fruit", background="black", foreground="red")
...
def gameStart(outputWindow):
    ...
    Thread(target= lambda:move(outputWindow)).start()
    Thread(target= lambda:timer(outputWindow)).start()
    display(outputWindow)
...
def main():
    ...
    outputWindow.config(relief=tkinter.SUNKEN, bg='black', fg='yellow', font=("Cascadia Mono", 12))
